...
func TestLoadRowsOnce_EdgeCases(t *testing.T) {
	// Test that loadRowsOnce can be called multiple times
	err := loadRowsOnce()
	if err != nil {
		t.Errorf("loadRowsOnce() error = %v", err)
	}

	// Call again to test idempotency
	err = loadRowsOnce()
	if err != nil {
		t.Errorf("loadRowsOnce() second call error = %v", err)
	}
}
...
func TestCalculate_EdgeCases(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tCelsius: 300.0, // Более низкая температура
			pPascal:  10e6,  // Высокое давление для сжатой жидкости
			wantErr:  false,
		},
		{
			name:     "High pressure",
...
func TestCalculate_TemperatureDependence(t *testing.T) {
	...
	pPascal := 10e6 // Высокое давление для сжатой жидкости
	temps := []float64{50.0, 100.0, 150.0, 200.0, 250.0, 300.0}
	...
	// Check that volume increases with temperature (for liquid water)
...
func TestCalculate_PressureDependence(t *testing.T) {
	...
	tCelsius := 200.0
	pressures := []float64{10e6, 20e6, 30e6, 40e6, 50e6, 100e6} // Все давления выше давления насыщения
	...
	// Check that volume decreases with pressure (for liquid water)
	for i := 1; i < len(volumes); i++ {
		if volumes[i] >= volumes[i-1] {
